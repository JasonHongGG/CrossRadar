...
void main() {
  ...
  test(
    'reuses today timetable file cache only for the Taipei service date',
    () async {
      ...
      final transport = _FakeTransport(
        trainTimetables: [_timetableRow('network')],
      );
  ...
  test(
    'rejects stale service-date timetable cache and replaces it after network fetch',
    () async {
      final now = DateTime.now().toUtc();
      SharedPreferences.setMockInitialValues({
        'tdx_cache_today_timetables': jsonEncode({
      ...
      final persisted =
          jsonDecode(preferences.getString('tdx_cache_today_timetables')!)
      ...
      InterceptorsWrapper(
        onRequest: (options, handler) {
          ...
          handler.resolve(
            Response(
              requestOptions: options,
              data: {'TrainTimetables': trainTimetables},
            ),
          );
        },
      ),
