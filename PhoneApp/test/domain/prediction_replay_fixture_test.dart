...
void main() {
  test(
    'replays the WebApp parity contract fixture through the PhoneApp predictor',
    () {
      final fixture =
          jsonDecode(
                File(
                  'test/fixtures/prediction_replay_fixture.json',
                ).readAsStringSync(),
              )
              as Map<String, dynamic>;
      final clock = RailwayClock(
        utcNow: () => DateTime.utc(2026, 5, 28, 1, 30),
      );
      final service = PredictionService(railwayClock: clock);
      final expected = fixture['expected'] as Map<String, dynamic>;

      final stations = {
        for (final stationJson in mapList(fixture['stations']))
          Station.fromJson(stationJson).id: Station.fromJson(stationJson),
      };
      final envelope = service.predictForCrossing(
        crossing: Crossing.fromJson(mapValue(fixture['crossing'])),
        timetables: mapList(
          fixture['timetables'],
        ).map(TrainTimetable.fromJson).toList(growable: false),
        liveboards: mapList(
          fixture['liveboards'],
        ).map(TrainLiveBoard.fromJson).toList(growable: false),
        trainInfos: mapList(
          fixture['train_info'],
        ).map(TrainInfo.fromJson).toList(growable: false),
        stationLookupById: stations,
        calibrationRules: const [],
        dataSnapshot: PredictionDataSnapshot.fromJson(
          mapValue(fixture['data_snapshot']),
        ),
      ...
      expect(
        prediction.segmentRatio,
        closeTo((expected['segment_ratio'] as num).toDouble(), 0.000001),
      );
      ...
      expect(
        trace.timeFraction,
        closeTo((expected['time_fraction'] as num).toDouble(), 0.000001),
      );
