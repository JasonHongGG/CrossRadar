...
void main() {
  ...
  test(
    'reverse direction applies 1 minus segment ratio and preserves crossing order',
    () {
      ...
      final north = service.predictForCrossing(
        crossing: _crossing('north', 0.3),
      ...
      final south = service.predictForCrossing(
        crossing: _crossing('south', 0.7),
        timetables: [timetable],
        liveboards: const [],
        trainInfos: const [],
        stationLookupById: _stations,
      ...
      expect(
        south.predictions.single.eta.isBefore(north.predictions.single.eta),
        isTrue,
      );
    },
  );
  ...
  test('runtime previous panel state starts empty and advances after eta', () {
    final prediction = PredictionRecord(
      trainNo: '3001',
      upstreamStationId: 'A',
      upstreamStationName: '甲站',
      downstreamStationId: 'B',
      downstreamStationName: '乙站',
      eta: _taipei(2026, 5, 28, 10),
      warning: true,
    ...
    final state = const PredictionRuntimeState().advance([
      prediction,
    ], _taipei(2026, 5, 28, 10, 0, 1));
  ...
  test('projects a liveboard station onto the active station pair', () {
    ...
    final snapshot = PredictionDataSnapshot(
      ...
      trainInfoCount: 0,
      sources: const [
        PredictionSnapshotSource(
          source: 'liveboards',
          fetchedFrom: 'stale_cache',
          recordCount: 1,
        ),
      ],
    );
    ...
    final envelope = service.predictForCrossing(
      ...
      liveboards: [
        TrainLiveBoard(
          ...
          stationId: 'C',
          stationName: '中間站',
          updateTime: _taipei(2026, 5, 28, 10, 6),
        ),
      ],
      trainInfos: const [],
      stationLookupById: _stations,
      calibrationRules: const [],
      stationPairProjections: const {
        'C|A|B': StationPairProjection(
          stationId: 'C',
          upstreamStationId: 'A',
          downstreamStationId: 'B',
          ratio: 0.4,
    ...
    expect(
      prediction.eta.difference(_taipei(2026, 5, 28, 10, 8)).inSeconds.abs(),
      lessThanOrEqualTo(5),
    );
  ...
  test('timetable fallback keeps liveboard projection rejection detail', () {
    ...
    final envelope = service.predictForCrossing(
      ...
      stationLookupById: _stations,
      calibrationRules: const [],
      stationPairProjectionRejections: const {
        'C|A|B': StationPairProjectionRejection(
          stationId: 'C',
          upstreamStationId: 'A',
          downstreamStationId: 'B',
          source: 'unavailable',
  ...
  test(
    'timetable fallback uses liveboard delay when only a downstream stop record exists',
    () {
      final service = PredictionService(railwayClock: clock);
      final now = _taipei(2026, 5, 28, 9, 45);
      ...
      final envelope = service.predictForCrossing(
        ...
        liveboards: [
          TrainLiveBoard(
            ...
            stationName: '臺南',
            delayTime: 1,
            updateTime: _taipei(2026, 5, 28, 10, 10),
          ),
        ],
        trainInfos: const [],
        stationLookupById: _stations,
      ...
      expect(
        prediction.reason,
        contains('no crossing-valid liveboard station context'),
      );
    },
  );

  test(
    'timetable fallback preserves zero-minute liveboard delay when projection is rejected',
    () {
      final service = PredictionService(railwayClock: clock);
      final now = _taipei(2026, 5, 28, 9, 45);
  ...
  test('mobile bundle decodes station pair projection rejections', () {
    ...
    expect(
      bundle.stationPairProjectionRejections['C|A|B']?.note,
      'No usable station-pair projection was exported for this liveboard station.',
    );
  });
  ...
  test('blocks predictions when a required snapshot source is incomplete', () {
    ...
    final envelope = service.predictForCrossing(
      ...
      dataSnapshot: const PredictionDataSnapshot(
        comprehensive: false,
        sources: [
          PredictionSnapshotSource(source: 'liveboards', complete: false),
        ],
      ),
  ...
  test('parses TDX update times without offset as Taipei railway time', () {
    final service = PredictionService(
      railwayClock: RailwayClock(
        utcNow: () => DateTime.utc(2026, 5, 28, 1, 45),
      ),
    );
    ...
    final envelope = service.predictForCrossing(
      ...
      stationLookupById: _stations,
      calibrationRules: const [],
      now: DateTime.utc(2026, 5, 28, 1, 45),
    );
  ...
  test(
    'applies calibration offsets and exposes timing trace parity fields',
    () {
      final service = PredictionService(railwayClock: clock);
      final base = service
          .predictForCrossing(
      ...
      expect(
        calibrated.trace?.timeFraction,
        closeTo(base.trace!.timeFraction, 0.000001),
      );
    },
  );
}
...
DateTime _taipei(
  ...
  int minute = 0,
  int second = 0,
]) => RailwayClock.instance.parseTimetableTime(
...
Crossing _crossing(String id, double ratio) => Crossing(
  ...
  stationA: const StationRef(
    id: 'A',
    name: '永康',
    position: GeoPoint(lat: 23.03825, lon: 120.25347),
  ),
  stationB: const StationRef(
    id: 'B',
    name: '臺南',
    position: GeoPoint(lat: 22.99681, lon: 120.21295),
  ),
  runtimeRatios: {
    'A|B': RuntimeRatio(
      upstreamStationId: 'A',
