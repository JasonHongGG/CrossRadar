...
class _PersistentRows {
  const _PersistentRows({required this.rows, required this.cachedAt});
...
class TdxTraClient {
  TdxTraClient({Dio? dio, this.basicBaseUrl = 'https://tdx.transportdata.tw/api/basic', this.tokenUrl = 'https://tdx.transportdata.tw/auth/realms/TDXConnect/protocol/openid-connect/token'}) : _dio = dio ?? Dio();
  ...
  Future<TdxCollectionResult<TrainTimetable>> getTodayTimetablesSnapshot(TdxCredentials credentials, {bool forceRefresh = false}) async {
    final result = await _getCollection(credentials, cacheKey: 'today_timetables', path: '/v3/Rail/TRA/DailyTrainTimetable/Today', collectionKey: 'TrainTimetables', ttl: const Duration(minutes: 30), forceRefresh: forceRefresh);
  ...
  Future<TdxCollectionResult<TrainInfo>> getTodayTrainInfosSnapshot(TdxCredentials credentials, {bool forceRefresh = false}) async {
    final result = await _getCollection(credentials, cacheKey: 'today_train_info', path: '/v2/Rail/TRA/DailyTrainInfo/Today', collectionKey: 'TrainInfos', ttl: const Duration(minutes: 5), forceRefresh: forceRefresh);
    return result.map((row) => TrainInfo.fromJson(row)).where((item) => item.trainNo.isNotEmpty).toResult();
  }

  Future<TdxCollectionResult<Map<String, dynamic>>> _getCollection(TdxCredentials credentials, {required String cacheKey, required String path, required String collectionKey, required Duration ttl, required bool forceRefresh}) async {
    final memory = _collectionCache[cacheKey];
    if (!forceRefresh && memory != null && memory.isValid) {
    ...
    final persistent = await _readPersistentRows(cacheKey);
    if (!forceRefresh && persistent != null && DateTime.now().difference(persistent.cachedAt) <= ttl) {
      _collectionCache[cacheKey] = _CacheEntry(value: persistent.rows, expiresAt: DateTime.now().add(ttl), cachedAt: persistent.cachedAt);
    ...
    try {
      ...
      await _writePersistentRows(cacheKey, cachedAt, rows);
  ...
  Future<_PersistentRows?> _readPersistentRows(String cacheKey) async {
    ...
    try {
      ...
      return _PersistentRows(rows: mapList(payload['data']), cachedAt: cachedAt);
    } on FormatException {
      return null;
    }
  }

  Future<void> _writePersistentRows(String cacheKey, DateTime cachedAt, List<Map<String, dynamic>> rows) async {
    final preferences = await SharedPreferences.getInstance();
    await preferences.setString('tdx_cache_$cacheKey', jsonEncode({'cached_at': cachedAt.toIso8601String(), 'data': rows}));
  }

  Future<String> _getAccessToken(TdxCredentials credentials, {bool forceRefresh = false}) async {
    if (!forceRefresh && _accessToken != null && _tokenExpiresAt != null && DateTime.now().add(const Duration(minutes: 5)).isBefore(_tokenExpiresAt!)) {
      return _accessToken!;
    }
