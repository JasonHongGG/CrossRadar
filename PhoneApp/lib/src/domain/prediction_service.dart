...
class PreparedTimetableCandidate {
  const PreparedTimetableCandidate({
    required this.timetable,
    required this.upstream,
    required this.downstream,
    required this.direction,
    required this.upstreamIndex,
    required this.downstreamIndex,
  });
...
class PreparedTimetableSet {
  const PreparedTimetableSet({
    required this.allCandidates,
    required this.byTrainNo,
  });
...
class _LivePredictionBuildResult {
  const _LivePredictionBuildResult({
    required this.predictions,
    required this.fallbackReasons,
    required this.fallbackDelays,
    required this.fallbackLiveboards,
  });
...
class _TimingEstimate {
  const _TimingEstimate({
    ...
    required this.etaUncertaintySeconds,
...
class PredictionRuntimeState {
  ...
  PredictionRuntimeState advance(List<PredictionRecord> tracked, DateTime now) {
    ...
    for (final prediction in tracked) {
      final key =
          '${prediction.identityKey}|${prediction.eta.toIso8601String()}';
...
class PredictionService {
  PredictionService({
    ...
    RailwayClock? railwayClock,
  }) : travelProfileService = travelProfileService ?? TravelProfileService(),
       railwayClock = railwayClock ?? RailwayClock.instance;
  ...
  PredictionEnvelope predictForCrossing({
    ...
    Map<String, StationPairProjectionRejection>
    ...
    now = railwayClock.normalizeTaipei(now ?? railwayClock.nowTaipei());
    dataSnapshot ??= PredictionDataSnapshot(
      liveboardCount: liveboards.length,
      delayedLiveboardCount: liveboards
          .where((item) => (item.delayTime ?? 0) != 0)
          .length,
      timetableCount: timetables.length,
      trainInfoCount: trainInfos.length,
      delayedTrainInfoCount: trainInfos
          .where((item) => (item.delayTime ?? 0) != 0)
          .length,
    );
    ...
    if (stationAId == null ||
      ...
      return _unavailable(
        ...
        horizonMinutes,
        recentMinutes,
        'station_pair_unresolved',
        'Station pair is unavailable.',
    ...
    if (crossing.ratioSource != 'osm_path' || crossing.segmentRatio == null) {
      return _unavailable(
        ...
        horizonMinutes,
        recentMinutes,
        'runtime_segment_unavailable',
        crossing.segmentConfidenceReason ??
            'No accepted OSM runtime segment is available.',
        dataSnapshot: dataSnapshot,
      );
    }
    if (!dataSnapshot.comprehensive) {
      final snapshot = _snapshotWithTiming(
        dataSnapshot,
        'prediction_total',
      ...
      return _unavailable(
        ...
        recentMinutes,
        'snapshot_incomplete',
        _snapshotIncompleteDetail(dataSnapshot),
        dataSnapshot: snapshot,
      );
    }

    final prepared = prepareTimetablesForCrossing(
      timetables,
      stationAId,
      stationBId,
      stationLookupById,
    ...
    final liveResult = _buildPredictionsFromLiveboards(
      crossing,
      liveboards,
      prepared,
      trainInfoByNo,
      stationLookupById,
      stationPairProjections,
      stationPairProjectionRejections,
      calibrationRules,
      now,
      horizonMinutes,
    ...
    final timetablePredictions = _buildPredictionsFromTimetables(
      crossing,
      prepared,
      trainInfoByNo,
      stationLookupById,
      ...
      recentMinutes,
      warningMinutes,
      liveboardFallbackReasons: liveResult.fallbackReasons,
      liveboardFallbackDelays: liveResult.fallbackDelays,
      liveboardFallbackLiveboards: liveResult.fallbackLiveboards,
    );
    final predictions = _dedupePredictions(
      _mergePredictions(liveResult.predictions, timetablePredictions),
    );
    final snapshot = _snapshotWithTiming(
      dataSnapshot,
      'prediction_total',
    ...
    if (predictions.isEmpty) {
      return _unavailable(
        ...
        horizonMinutes,
        recentMinutes,
        'no_prediction_candidates',
        liveResult.fallbackReasons.values.firstOrNull ??
            'No timetable or liveboard candidates matched this crossing.',
        dataSnapshot: snapshot,
      );
    }
    ...
    return PredictionEnvelope(
      ...
      available: true,
      dataSnapshot: snapshot,
      recentPrediction: partition.$1,
      upcomingPredictions: partition.$2,
      predictions: partition.$3,
    );
  }

  PreparedTimetableSet prepareTimetablesForCrossing(
    List<TrainTimetable> timetables,
    String stationAId,
    String stationBId,
    Map<String, Station> stationLookupById,
  ) {
    final all = <PreparedTimetableCandidate>[];
    final byTrainNo = <String, List<PreparedTimetableCandidate>>{};
    for (final timetable in timetables) {
      final pair = resolveStopPair(
        timetable,
        stationAId,
        stationBId,
        stationLookupById,
      ...
      final candidate = PreparedTimetableCandidate(
        timetable: timetable,
        upstream: pair.$1,
        downstream: pair.$2,
        direction: pair.$3,
    ...
    for (final candidates in byTrainNo.values) {
      candidates.sort(
        (a, b) => _candidateScore(
          ...
          stationAId,
          stationBId,
        ).compareTo(_candidateScore(a, stationAId, stationBId)),
      );
    }
    return PreparedTimetableSet(allCandidates: all, byTrainNo: byTrainNo);
  }

  (StopTime, StopTime, int)? resolveStopPair(
    ...
    final stopA = timetable.stopTimes
        .where((stop) => stop.stationId == stationAId)
        .firstOrNull;
    final stopB = timetable.stopTimes
        .where((stop) => stop.stationId == stationBId)
        .firstOrNull;
    if (stopA == null || stopB == null) {
      if (stopA != null) {
        return _resolveSingleAnchorStopPair(
          timetable.stopTimes,
          anchorStop: stopA,
          anchorStationId: stationAId,
          targetStationId: stationBId,
          stationLookupById: stationLookupById,
          anchorRole: 'a',
          trainDirection: timetable.direction,
        );
      }
      if (stopB != null) {
        return _resolveSingleAnchorStopPair(
          timetable.stopTimes,
          anchorStop: stopB,
          anchorStationId: stationBId,
          targetStationId: stationAId,
          stationLookupById: stationLookupById,
          anchorRole: 'b',
          trainDirection: timetable.direction,
        );
  ...
  List<PredictionRecord> _buildPredictionsFromTimetables(
    Crossing crossing,
    PreparedTimetableSet prepared,
    Map<String, TrainInfo> trainInfoByNo,
    Map<String, Station> stationLookupById,
    List<CalibrationRule> calibrationRules,
    DateTime now,
    int? horizonMinutes,
    int recentMinutes,
    int warningMinutes, {
    Map<String, String> liveboardFallbackReasons = const {},
    Map<String, DelayEstimate> liveboardFallbackDelays = const {},
    Map<String, TrainLiveBoard> liveboardFallbackLiveboards = const {},
  }) {
    final predictions = <PredictionRecord>[];
    for (final candidate in prepared.allCandidates) {
      final timetable = candidate.timetable;
      final upstreamDeparture = parseTimeOnDate(
        now,
        candidate.upstream.departureTime ?? candidate.upstream.arrivalTime,
      );
      final downstreamArrival = parseTimeOnDate(
        now,
        candidate.downstream.arrivalTime ?? candidate.downstream.departureTime,
      );
      if (upstreamDeparture == null ||
          downstreamArrival == null ||
          !downstreamArrival.isAfter(upstreamDeparture)) {
        continue;
      }
      final delay =
          liveboardFallbackDelays[timetable.trainNo] ??
          _resolveDelayMinutes(timetable.trainNo, trainInfoByNo: trainInfoByNo);
      ...
      predictions.add(
        _buildRecord(
          ...
          hasLiveboard: false,
          delay: delay,
          liveboard: liveboardFallbackLiveboards[timetable.trainNo],
          scheduledUpstream: upstreamDeparture,
          scheduledDownstream: downstreamArrival,
          fallbackReason: liveboardFallbackReasons[timetable.trainNo],
        ),
      );
    }
  ...
  _LivePredictionBuildResult _buildPredictionsFromLiveboards(
    Crossing crossing,
    List<TrainLiveBoard> liveboards,
    PreparedTimetableSet prepared,
    Map<String, TrainInfo> trainInfoByNo,
    Map<String, Station> stationLookupById,
    Map<String, StationPairProjection> stationPairProjections,
    Map<String, StationPairProjectionRejection> stationPairProjectionRejections,
    List<CalibrationRule> calibrationRules,
    DateTime now,
    int? horizonMinutes,
    int recentMinutes,
    ...
    for (final entry in prepared.byTrainNo.entries) {
      ...
      final selection = _selectLiveboardCandidateContext(
        crossing,
        trainLiveboards,
        entry.value,
        trainInfoByNo,
      ...
      final timing = _estimatePredictionTiming(
        ...
        trainNo: timetable.trainNo,
        trainTypeName:
            timetable.trainTypeName ?? context.liveboard.trainTypeName,
        upstreamStationId: candidate.upstream.stationId,
        downstreamStationId: candidate.downstream.stationId,
        upstream: candidate.upstream,
        downstream: candidate.downstream,
    ...
    return _LivePredictionBuildResult(
      predictions: predictions,
      fallbackReasons: fallbackReasons,
      fallbackDelays: fallbackDelays,
      fallbackLiveboards: fallbackLiveboards,
    );
  }

  PredictionRecord _buildRecord(
    Crossing crossing,
    TrainTimetable timetable,
    PreparedTimetableCandidate candidate,
    _TimingEstimate timing,
    int warningMinutes, {
    required DateTime now,
    required bool hasLiveboard,
    required DelayEstimate delay,
    required TrainLiveBoard? liveboard,
    required DateTime scheduledUpstream,
    required DateTime scheduledDownstream,
    ...
    return PredictionRecord(
      ...
      sourceStationName:
          liveboard?.stationName ?? candidate.upstream.stationName,
      ...
      confidence: _predictionConfidence(
        crossing.geolocationConfidence,
        timing.segmentConfidence,
        hasLiveboard: hasLiveboard,
      ),
      confidenceReason:
          'Timing source: ${hasLiveboard ? 'liveboard' : 'timetable'}. Ratio source: ${timing.ratioSource}. ${timing.segmentNote}',
      ...
      trace: PredictionTrace(
        serviceDate: railwayClock.serviceDateKey(now),
        trainNo: timetable.trainNo,
        upstreamStationId: candidate.upstream.stationId,
        downstreamStationId: candidate.downstream.stationId,
        ratio: timing.ratio,
        ratioSource: timing.ratioSource,
        segmentConfidence: timing.segmentConfidence,
        scheduledUpstream: scheduledUpstream,
        scheduledDownstream: scheduledDownstream,
        actualUpstream: timing.actualUpstream,
        actualDownstream: timing.actualDownstream,
        delaySeconds: delay.seconds,
        delaySource: delay.source,
        travelProfileId: timing.profile.profileId,
        trainTypeFamily: timing.profile.trainTypeFamily,
        timeFraction: timing.profile.timeFraction,
        timingModel: timing.timingModel,
        anchorTimeSource: timing.anchorTimeSource,
        calibrationOffsetSeconds: timing.calibrationOffsetSeconds,
        eta: timing.eta,
        liveboardStationId: liveboard?.stationId,
        liveboardUpdateTime: liveboard?.updateTime,
  ...
  _TimingEstimate? _estimatePredictionTiming(
    ...
    final context = _predictionSegmentContext(
      crossing,
      upstreamStationId,
      downstreamStationId,
    );
    ...
    var actualDownstream = downstreamArrival.add(
      Duration(seconds: delaySeconds),
    );
    final profile = travelProfileService.estimate(
      ratio: ratio,
      trainTypeName: trainTypeName,
      upstreamDwellSeconds: _stopDwellSeconds(upstream, upstreamDeparture),
      downstreamDwellSeconds: _stopDwellSeconds(downstream, downstreamArrival),
    );
    var eta = actualUpstream.add(
      _scaleDuration(scheduledDuration, profile.timeFraction),
    );
    var timingModel = 'travel_profile';
    String? anchorTimeSource;
    final anchor = _resolveLiveboardAnchorTime(
      liveboard,
      upstreamStationId,
      actualUpstream,
      actualDownstream,
    ...
    final uncertainty = _estimateUncertaintySeconds(
      baseUncertaintySeconds: profile.baseUncertaintySeconds,
      segmentConfidence: context.confidence,
      dataBasis: dataBasis,
    ...
    return _TimingEstimate(
      eta: eta,
      ratio: ratio,
      ratioSource: context.source,
      segmentConfidence: context.confidence,
      segmentNote: context.note,
      actualUpstream: actualUpstream,
      actualDownstream: actualDownstream,
      timingModel: timingModel,
      anchorTimeSource: anchorTimeSource,
      calibrationOffsetSeconds: calibration,
      etaUncertaintySeconds: uncertainty,
      accuracyTier: _accuracyTierFromUncertainty(uncertainty),
      profile: profile,
    );
  }

  _SegmentContext _predictionSegmentContext(
    Crossing crossing,
    String upstreamStationId,
    String downstreamStationId,
  ) {
    final stationAId = crossing.stationA.id;
    final stationBId = crossing.stationB.id;
    if (crossing.segmentRatio != null &&
        crossing.ratioSource == 'osm_path' &&
        upstreamStationId == stationAId &&
        downstreamStationId == stationBId) {
      return _SegmentContext(
        ratio: crossing.segmentRatio,
        source: 'osm_path',
        confidence: crossing.segmentConfidence ?? 'high',
        note: crossing.segmentConfidenceReason ?? 'OSM runtime segment.',
      );
    }
    if (crossing.segmentRatio != null &&
        crossing.ratioSource == 'osm_path' &&
        upstreamStationId == stationBId &&
        downstreamStationId == stationAId) {
      return _SegmentContext(
        ratio: 1.0 - crossing.segmentRatio!,
        source: 'osm_path',
        confidence: crossing.segmentConfidence ?? 'high',
        note: 'Reverse direction OSM runtime segment.',
      );
    }
    final ratio =
        crossing.runtimeRatios['$upstreamStationId|$downstreamStationId'];
    if (ratio != null && ratio.source == 'osm_path') {
      return _SegmentContext(
        ratio: ratio.ratio,
        source: ratio.source,
        confidence: ratio.confidence,
        note: ratio.note ?? 'Precomputed OSM stop-pair ratio.',
      );
    }
    return const _SegmentContext(
      ratio: null,
      source: 'unavailable',
      confidence: 'low',
      note: 'No OSM ratio for this stop pair.',
    );
  }

  bool _isPredictionSegmentValid(
    Crossing crossing,
    String upstreamStationId,
    String downstreamStationId,
    double ratio,
    ...
    final stationAId = crossing.stationA.id;
    final stationBId = crossing.stationB.id;
    if ((upstreamStationId == stationAId &&
            downstreamStationId == stationBId) ||
        (upstreamStationId == stationBId &&
            downstreamStationId == stationAId)) {
      return true;
  ...
  _LiveSelection _selectLiveboardCandidateContext(
    Crossing crossing,
    List<TrainLiveBoard> liveboards,
    List<PreparedTimetableCandidate> candidates,
    Map<String, TrainInfo> trainInfoByNo,
    Map<String, StationPairProjection> stationPairProjections,
    Map<String, StationPairProjectionRejection> stationPairProjectionRejections,
    ...
    for (final liveboard in liveboards) {
      for (final candidate in candidates) {
        final observedIndex = _findTimetableStopIndex(
          candidate.timetable.stopTimes,
          liveboard.stationId,
          candidate,
        );
        ...
        late int phaseRank;
        late int stopGap;
        final pairSpan = math.max(
          candidate.downstreamIndex - candidate.upstreamIndex,
          0,
        );
        final liveRank = -(liveboard.updateTime?.millisecondsSinceEpoch ?? 0);
        if (observedIndex == null) {
          final projection = _projectLiveboardStationRatio(
            liveboard,
            candidate,
            stationPairProjections,
            stationPairProjectionRejections,
          ...
          if (projection.ratio == null) {
            if (fallbackScore == null ||
                _compareScore(failureScore, fallbackScore) < 0) {
          ...
          final crossingRatio = _predictionSegmentContext(
            crossing,
            candidate.upstream.stationId,
            candidate.downstream.stationId,
          ).ratio;
          if (crossingRatio == null) {
            if (fallbackScore == null ||
                _compareScore(failureScore, fallbackScore) < 0) {
              fallbackScore = failureScore;
              fallbackReason = _formatLiveboardFallbackReason(
                liveboard,
                candidate,
                'No OSM stop-pair ratio is available for this station pair.',
              );
            }
            continue;
          }
          ...
          delay = _resolveProjectedLiveboardDelayEstimate(
            candidate,
            liveboard,
            projectedRatio,
            trainInfoByNo,
          ...
          delay = _resolveLiveboardDelayEstimate(
            candidate.timetable.trainNo,
            liveboard,
            observedStop,
            trainInfoByNo,
            referenceDate,
    ...
    return _LiveSelection(
      context: best,
      fallbackReason: best == null ? fallbackReason : null,
    );
  }

  _StationProjectionResolution _projectLiveboardStationRatio(
    TrainLiveBoard liveboard,
    PreparedTimetableCandidate candidate,
    Map<String, StationPairProjection> stationPairProjections,
    Map<String, StationPairProjectionRejection> stationPairProjectionRejections,
    ...
    final key =
        '${liveboard.stationId}|${candidate.upstream.stationId}|${candidate.downstream.stationId}';
    final projection = stationPairProjections[key];
    final stationLabel = liveboard.stationName ?? liveboard.stationId;
    final pairLabel =
        '${candidate.upstream.stationName}-${candidate.downstream.stationName}';
    if (projection != null) {
      if (projection.source != 'osm_path') {
        return _StationProjectionResolution(
          reason:
              '$stationLabel could not use the exported projection for $pairLabel: ${projection.note ?? 'projection source is not OSM-backed.'}',
        );
      }
      if (projection.ratio <= 0.0 || projection.ratio >= 1.0) {
        return _StationProjectionResolution(
          reason:
              '$stationLabel could not use the exported projection for $pairLabel: ${projection.note ?? 'projection ratio is outside the usable range.'}',
        );
      }
      return _StationProjectionResolution(
        ratio: projection.ratio,
        reason:
            'Projected $stationLabel onto $pairLabel via the mobile OSM stop-pair map.',
      );
    }
    final rejection = stationPairProjectionRejections[key];
    return _StationProjectionResolution(
      reason: _formatLiveboardFallbackReason(
        liveboard,
        candidate,
        rejection?.note ??
            'No usable station-pair projection was exported for this liveboard station.',
      ),
    );
  }

  String _formatLiveboardFallbackReason(
    TrainLiveBoard liveboard,
    PreparedTimetableCandidate candidate,
    String detail,
  ) {
  ...
  int? _findTimetableStopIndex(
    ...
    indexes.sort((a, b) {
      ...
      return (candidate.upstreamIndex - a).abs().compareTo(
        (candidate.upstreamIndex - b).abs(),
      );
    });
    return indexes.first;
  }

  DelayEstimate _resolveLiveboardDelayEstimate(
    String trainNo,
    TrainLiveBoard liveboard,
    StopTime observedStop,
    Map<String, TrainInfo> trainInfoByNo,
    ...
    final scheduled = parseTimeOnDate(
      referenceDate,
      observedStop.departureTime ?? observedStop.arrivalTime,
    );
    ...
    if (scheduled != null && observedAt != null) {
      ...
      if (delta >= -300 && delta <= 7200) {
        return DelayEstimate(
          seconds: delta,
          minutes: (delta / 60).round(),
          source: 'liveboard',
          reason:
              'Accepted the liveboard update-time delta against the matched timetable stop.',
        );
      }
    }
    if (liveboard.delayTime != null) {
      return DelayEstimate(
        seconds: liveboard.delayTime! * 60,
        minutes: liveboard.delayTime!,
        source: 'liveboard',
        reason:
            'Accepted the train-level TrainLiveBoard DelayTime because the stop-time delta was outside the usable window.',
      );
    }
    final info = trainInfoByNo[trainNo];
    if (info?.delayTime != null) {
      return DelayEstimate(
        seconds: info!.delayTime! * 60,
        minutes: info.delayTime!,
        source: 'train_info',
        reason:
            'Fell back to TrainInfo DelayTime because TrainLiveBoard did not provide a usable delay signal.',
      );
    }
    return const DelayEstimate(
      seconds: 0,
      minutes: 0,
      source: 'none',
      reason:
          'No usable TrainLiveBoard or TrainInfo delay signal was available for this matched station context.',
    );
  }

  DelayEstimate _resolveProjectedLiveboardDelayEstimate(
    PreparedTimetableCandidate candidate,
    TrainLiveBoard liveboard,
    double observedRatio,
    Map<String, TrainInfo> trainInfoByNo,
    ...
    if (upstreamDeparture != null &&
      ...
      final profile = travelProfileService.estimate(
        ratio: observedRatio,
        trainTypeName:
            candidate.timetable.trainTypeName ?? liveboard.trainTypeName,
        upstreamDwellSeconds: _stopDwellSeconds(
          candidate.upstream,
          referenceDate,
        ),
        downstreamDwellSeconds: _stopDwellSeconds(
          candidate.downstream,
          referenceDate,
        ),
      );
      final scheduledObserved = upstreamDeparture.add(
        _scaleDuration(
          downstreamArrival.difference(upstreamDeparture),
          profile.timeFraction,
        ),
      );
      ...
      if (delta >= -300 && delta <= 7200) {
        return DelayEstimate(
          ...
          source: 'liveboard',
          reason:
              'Accepted the projected TrainLiveBoard update-time delta on the active OSM stop-pair.',
        );
      }
    }
    return _resolveLiveboardDelayEstimate(
      candidate.timetable.trainNo,
      liveboard,
      candidate.upstream,
      trainInfoByNo,
  ...
  DelayEstimate _resolveDelayMinutes(
    String trainNo, {
    Map<String, TrainInfo>? trainInfoByNo,
  }) {
    final info = trainInfoByNo?[trainNo];
    if (info?.delayTime != null) {
      return DelayEstimate(
        ...
        source: 'train_info',
        reason:
            'Used TrainInfo DelayTime because no liveboard evidence was selected for this train.',
      );
    }
    return const DelayEstimate(
      seconds: 0,
      ...
      source: 'none',
      reason:
          'No liveboard or train-info delay evidence was available for this train.',
    );
  }

  (DelayEstimate, TrainLiveBoard)? _resolveLiveboardDelayFallback(
    List<TrainLiveBoard> liveboards,
  ) {
    for (final liveboard in liveboards) {
      ...
      return (
        DelayEstimate(
          ...
          source: 'liveboard',
          reason:
              'Used TrainLiveBoard DelayTime even though no crossing-valid liveboard station context was available.',
        ),
        liveboard,
      );
    }
  ...
  Map<String, List<TrainLiveBoard>> _buildLiveboardIndex(
    List<TrainLiveBoard> liveboards,
  ) {
    ...
    for (final liveboard in liveboards) {
      ...
      if (current == null ||
          (liveboard.updateTime?.millisecondsSinceEpoch ?? -1) >=
              (current.updateTime?.millisecondsSinceEpoch ?? -1)) {
    ...
    for (final records in index.values) {
      records.sort(
        (a, b) => (b.updateTime?.millisecondsSinceEpoch ?? -1).compareTo(
          a.updateTime?.millisecondsSinceEpoch ?? -1,
        ),
      );
    }
    return index;
  ...
  (StopTime, StopTime, int)? _resolveSingleAnchorStopPair(
    List<StopTime> stops, {
    required StopTime anchorStop,
    required String anchorStationId,
    required String targetStationId,
    required Map<String, Station> stationLookupById,
    required String anchorRole,
    required int? trainDirection,
  }) {
    ...
    final indexes = [
      anchorIndex - 1,
      anchorIndex + 1,
    ].where((index) => index >= 0 && index < stops.length).toList();
    final bestIndex = _pickNeighborTowardTarget(
      stops,
      anchorStationId,
      targetStationId,
      indexes,
    ...
    if (bestIndex < anchorIndex) {
      return (
        candidate,
        anchorStop,
        trainDirection ?? (anchorRole == 'b' ? 0 : 1),
      );
    }
    return (
      anchorStop,
      candidate,
      trainDirection ?? (anchorRole == 'a' ? 0 : 1),
    );
  }

  int? _pickNeighborTowardTarget(
    List<StopTime> stops,
  ...
  double _distanceSq(GeoPoint a, GeoPoint b) =>
      math.pow(a.lat - b.lat, 2).toDouble() +
      math.pow(a.lon - b.lon, 2).toDouble();
  ...
  double _alignment(GeoPoint anchor, GeoPoint target, GeoPoint candidate) {
    ...
    final candidateVector = (
      candidate.lat - anchor.lat,
      candidate.lon - anchor.lon,
    );
    return targetVector.$1 * candidateVector.$1 +
        targetVector.$2 * candidateVector.$2;
  }

  DateTime? parseTimeOnDate(DateTime date, String? timeText) {
    return railwayClock.parseTimetableTime(date, timeText);
  }
  ...
  DateTime? _resolveLiveboardAnchorTime(
    TrainLiveBoard? liveboard,
    String upstreamStationId,
    DateTime scheduledUpstream,
    DateTime scheduledDownstream,
  ) {
    if (liveboard == null ||
        liveboard.stationId != upstreamStationId ||
        liveboard.updateTime == null) {
      return null;
    }
    final parsed = liveboard.updateTime!;
    if (parsed.isBefore(
          scheduledUpstream.subtract(const Duration(minutes: 5)),
        ) ||
        parsed.isAfter(scheduledDownstream.add(const Duration(minutes: 10)))) {
      return null;
    }
    return parsed.isBefore(scheduledUpstream) ? scheduledUpstream : parsed;
  }

  int _lookupOffsetSeconds(
    List<CalibrationRule> rules, {
    required String crossingId,
    required int direction,
    required String? trainTypeName,
    required String upstreamStationId,
  }) {
    ...
    for (final rule in rules) {
      ...
      if (match['direction'] != null &&
          intValue(match['direction']) != direction) {
        continue;
      }
      if (match['train_type_family'] != null &&
          match['train_type_family'] != family) {
        continue;
      }
      if (match['upstream_station_id'] != null &&
          match['upstream_station_id'] != upstreamStationId) {
        continue;
      }
  ...
  int _estimateUncertaintySeconds({
    required int baseUncertaintySeconds,
    required String segmentConfidence,
    required String dataBasis,
    required String delaySource,
    required String? anchorTimeSource,
  ...
  String _predictionConfidence(
    String? geoConfidence,
    String? segmentConfidence, {
    required bool hasLiveboard,
  }) {
    final rank = math.min(
      _confidenceRank(geoConfidence),
      _confidenceRank(segmentConfidence ?? geoConfidence),
    );
  ...
  (PredictionRecord?, List<PredictionRecord>, List<PredictionRecord>)
  _partitionPredictions(
    ...
    return (
      recent,
      upcoming.take(2).toList(growable: false),
      recentAndUpcoming,
    );
  }

  List<PredictionRecord> _mergePredictions(
    List<PredictionRecord> livePredictions,
    List<PredictionRecord> timetablePredictions,
  ) {
    final liveKeys = livePredictions
        .map((record) => record.identityKey)
        .toSet();
    return [
      ...livePredictions,
      ...timetablePredictions.where(
        (record) => !liveKeys.contains(record.identityKey),
      ),
    ];
  }

  List<PredictionRecord> _dedupePredictions(
    List<PredictionRecord> predictions,
  ) {
    ...
    for (final prediction in predictions) {
      ...
      if (current == null ||
          _predictionPreference(
                prediction,
              ).compareTo(_predictionPreference(current)) <
              0) {
    ...
    values.sort((a, b) {
      ...
      if (eta != 0) return eta;
      final basis = (a.dataBasis == 'liveboard' ? 0 : 1).compareTo(
        b.dataBasis == 'liveboard' ? 0 : 1,
      );
  ...
  (int, int, int, int) _predictionPreference(PredictionRecord prediction) {
    ...
    final sourceRank =
        prediction.sourceStationId == prediction.upstreamStationId ||
            prediction.sourceStationId == prediction.downstreamStationId
        ? 0
        : 1;
  ...
  (int, int, int) _candidateScore(
    ...
    final exact =
        pairIds.length == anchorIds.length && pairIds.containsAll(anchorIds)
        ? 1
        : 0;
    final anchors =
        (anchorIds.contains(candidate.upstream.stationId) ? 1 : 0) +
        (anchorIds.contains(candidate.downstream.stationId) ? 1 : 0);
    final gap =
        (candidate.downstream.stopSequence - candidate.upstream.stopSequence)
            .abs();
  ...
  PredictionDataSnapshot _snapshotWithTiming(
    PredictionDataSnapshot snapshot,
    String key,
    int elapsedMs,
  ) {
    return PredictionDataSnapshot(
      comprehensive: snapshot.comprehensive,
      liveboardCount: snapshot.liveboardCount,
      delayedLiveboardCount: snapshot.delayedLiveboardCount,
      timetableCount: snapshot.timetableCount,
      trainInfoCount: snapshot.trainInfoCount,
      delayedTrainInfoCount: snapshot.delayedTrainInfoCount,
      liveboardScope: snapshot.liveboardScope,
      sources: snapshot.sources,
      timingsMs: {...snapshot.timingsMs, key: elapsedMs},
    );
  }

  String _snapshotIncompleteDetail(PredictionDataSnapshot snapshot) {
    final incomplete = snapshot.sources
        .where((source) => !source.complete)
        .map((source) => source.source)
        .toList();
    if (incomplete.isEmpty) return 'Prediction snapshot is incomplete.';
    return 'Prediction snapshot is incomplete: ${incomplete.join(', ')}.';
  }

  PredictionEnvelope _unavailable(
    Crossing crossing,
    DateTime now,
    int warningMinutes,
    int? horizonMinutes,
