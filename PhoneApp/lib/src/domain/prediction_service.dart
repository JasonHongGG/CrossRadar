...
class PredictionService {
  PredictionService({TravelProfileService? travelProfileService, RailwayClock? railwayClock}) : travelProfileService = travelProfileService ?? TravelProfileService(), railwayClock = railwayClock ?? RailwayClock.instance;
  ...
  PredictionEnvelope predictForCrossing({
    ...
    if (!dataSnapshot.comprehensive) {
      ...
      return _unavailable(crossing, now, warningMinutes, horizonMinutes, recentMinutes, 'snapshot_incomplete', _snapshotIncompleteDetail(dataSnapshot), dataSnapshot: snapshot);
    }
  ...
  List<PredictionRecord> _buildPredictionsFromTimetables(Crossing crossing, PreparedTimetableSet prepared, Map<String, TrainInfo> trainInfoByNo, Map<String, Station> stationLookupById, List<CalibrationRule> calibrationRules, DateTime now, int? horizonMinutes, int recentMinutes, int warningMinutes, {Map<String, String> liveboardFallbackReasons = const {}, Map<String, DelayEstimate> liveboardFallbackDelays = const {}, Map<String, TrainLiveBoard> liveboardFallbackLiveboards = const {}}) {
    ...
    for (final candidate in prepared.allCandidates) {
      ...
      predictions.add(_buildRecord(crossing, timetable, candidate, timing, warningMinutes, now: now, hasLiveboard: false, delay: delay, liveboard: liveboardFallbackLiveboards[timetable.trainNo], scheduledUpstream: upstreamDeparture, scheduledDownstream: downstreamArrival, fallbackReason: liveboardFallbackReasons[timetable.trainNo]));
    }
  ...
  _LivePredictionBuildResult _buildPredictionsFromLiveboards(Crossing crossing, List<TrainLiveBoard> liveboards, PreparedTimetableSet prepared, Map<String, TrainInfo> trainInfoByNo, Map<String, Station> stationLookupById, Map<String, StationPairProjection> stationPairProjections, Map<String, StationPairProjectionRejection> stationPairProjectionRejections, List<CalibrationRule> calibrationRules, DateTime now, int? horizonMinutes, int recentMinutes, int warningMinutes) {
    ...
    for (final entry in prepared.byTrainNo.entries) {
      ...
      predictions.add(_buildRecord(crossing, timetable, candidate, timing, warningMinutes, now: now, hasLiveboard: true, delay: context.delayEstimate, liveboard: context.liveboard, scheduledUpstream: upstreamDeparture, scheduledDownstream: downstreamArrival, observedRatio: context.observedRatio, projectionReason: context.projectionReason));
    }
  ...
  PredictionRecord _buildRecord(Crossing crossing, TrainTimetable timetable, PreparedTimetableCandidate candidate, _TimingEstimate timing, int warningMinutes, {required DateTime now, required bool hasLiveboard, required DelayEstimate delay, required TrainLiveBoard? liveboard, required DateTime scheduledUpstream, required DateTime scheduledDownstream, double? observedRatio, String? projectionReason, String? fallbackReason}) {
  ...
  String _snapshotIncompleteDetail(PredictionDataSnapshot snapshot) {
    final incomplete = snapshot.sources.where((source) => !source.complete).map((source) => source.source).toList();
