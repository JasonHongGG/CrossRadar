...
class StationRef {
  ...
  Map<String, Object?> toJson() => {'id': id, 'name': name, 'position': position?.toJson(), 'uk_primary': ukPrimary, 'uk_values': ukValues};
}
...
class RuntimeRatio {
  ...
  Map<String, Object?> toJson() => {'upstream_station_id': upstreamStationId, 'downstream_station_id': downstreamStationId, 'ratio': ratio, 'source': source, 'confidence': confidence, 'note': note};
}
...
class Crossing {
  ...
  Map<String, Object?> toJson() => {
    ...
    'runtime_ratios': runtimeRatios.map((key, value) => MapEntry(key, value.toJson())),
...
class MobileBundle {
  const MobileBundle({required this.metadata, required this.crossings, required this.stations, required this.calibrationRules, this.calibrationMetadata = const {}, this.stationPairProjections = const {}, this.stationPairProjectionRejections = const {}});
  ...
  Map<String, dynamic> get predictionContract => mapValue(metadata['prediction_contract']);
...
class TrainLiveBoard {
  ...
  factory TrainLiveBoard.fromJson(Map<String, dynamic> json) {
    ...
    return TrainLiveBoard(trainNo: textValue(json['TrainNo']) ?? '', stationId: textValue(json['StationID']) ?? '', stationName: textValue(json['StationName']), trainTypeName: textValue(json['TrainTypeName']), updateTime: RailwayClock.instance.parseTdxUpdateTime(rawUpdate), delayTime: json['DelayTime'] == null ? null : intValue(json['DelayTime']));
  }
}
...
class PredictionSnapshotSource {
  ...
  Map<String, Object?> toJson() => {'source': source, 'complete': complete, 'record_count': recordCount, 'delayed_record_count': delayedRecordCount, 'fetched_from': fetchedFrom, 'cached_at': cachedAt?.toIso8601String(), 'scope': scope, 'detail': detail, 'timing_breakdown': timingBreakdown};
}
...
class PredictionDataSnapshot {
  ...
  Map<String, Object?> toJson() => {'comprehensive': comprehensive, 'liveboard_count': liveboardCount, 'delayed_liveboard_count': delayedLiveboardCount, 'timetable_count': timetableCount, 'train_info_count': trainInfoCount, 'delayed_train_info_count': delayedTrainInfoCount, 'liveboard_scope': liveboardScope, 'sources': sources.map((source) => source.toJson()).toList(), 'timings_ms': timingsMs};
}
...
class PredictionEnvelope {
  ...
  Map<String, Object?> toJson() => {
    ...
    'upcoming_predictions': upcomingPredictions.map((prediction) => prediction.toJson()).toList(),
